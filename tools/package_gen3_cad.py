"""Create and verify my deterministic Gen3 STEP/STL download archives."""
...
def build_archive(kind: str, manifest: dict) -> dict:
    archive = OUTPUT / f"Bolley_Gen3_{kind.upper()}.zip"
    records = []
    with zipfile.ZipFile(archive, "w") as handle:
        for part_name, part in sorted(manifest["artifacts"].items()):
            source = ROOT / part[kind]["path"]
            if not source.exists():
                raise SystemExit(f"missing {kind.upper()} source; run cad/build_gen3.py --write")
            payload = source.read_bytes()
            info = zipfile.ZipInfo(source.name, date_time=FIXED_ZIP_TIME)
...
def write() -> None:
    ...
    packages = {kind: build_archive(kind, manifest) for kind in ("step", "stl")}
    payload = {
        "schema_version": 1,
        "generation": "Gen3",
...
def check() -> None:
    if not MANIFEST.exists():
        raise SystemExit("missing cad/exports/gen3/PACKAGE.json")
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["build_manifest_sha256"] != sha256(BUILD):
        raise SystemExit("Gen3 CAD package is stale relative to BUILD_GEN3.json")
    for package in manifest["packages"].values():
        archive = ROOT / package["path"]
        if not archive.exists() or archive.stat().st_size != package["bytes"] or sha256(archive) != package["sha256"]:
            raise SystemExit(f"stale Gen3 archive: {archive.relative_to(ROOT)}")
        with zipfile.ZipFile(archive) as handle:
            ...
            for record in package["members"]:
                ...
                if len(payload) != record["bytes"] or sha256_bytes(payload) != record["sha256"]:
                    raise SystemExit(f"member mismatch: {archive.relative_to(ROOT)}::{record['archive_member']}")
    print("OK: deterministic Gen3 STEP/STL archives match PACKAGE.json")
