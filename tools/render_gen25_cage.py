"""Render A7a cage/circuit corner and CG evidence."""
...
RESULT = ROOT / "analysis" / "results" / "gen25_cage_circuit.json"
FIGURE_DIR = ROOT / "analysis" / "figures" / "a7a"
...
def render() -> None:
    ...
    path = FIGURE_DIR / "A7a_qualification_cage_rise_map.png"
    ...
    contour = ax.contour(
        np.asarray(y_values) * 1e3,
        np.asarray(z_values) * 1e3,
        cage_rise,
        levels=[20.0],
        colors=["white"],
        linewidths=1.4,
    )
    ax.clabel(contour, fmt={20.0: "20 K limit"}, inline=True, fontsize=9)
    ax.set_title("A7a qualification cage rise | 90% conductance")
    colorbar = fig.colorbar(image, ax=ax, pad=0.02)
    colorbar.set_label("Maximum channel cage rise [K]")
    fig.tight_layout()
    fig.savefig(path, metadata={"Software": "Bolley deterministic A7a renderer"})
    plt.close(fig)
    ...
    path = FIGURE_DIR / "A7a_controlling_margins.png"
    ...
    fig.suptitle("A7a controlling corner margins | <=1 passes")
    fig.tight_layout()
    fig.savefig(path, metadata={"Software": "Bolley deterministic A7a renderer"})
    plt.close(fig)
    figures.append((path, "A7a controlling normalized corner margins", "MODEL OUTPUT"))
    ...
    records = [
        {
            ...
            "sha256": sha256(path),
            "source": "tools/render_gen25_cage.py",
        }
        for path, title, evidence in figures
    ]
    MANIFEST.write_text(
        json.dumps(
            {"schema_version": 1, "gate": "A7a", "figure_count": len(records), "figures": records},
            indent=2,
            sort_keys=True,
        )
        + "\n",
...
def check() -> None:
    if not MANIFEST.exists():
        raise SystemExit("missing A7a figure manifest")
    ...
    for record in manifest["figures"]:
        ...
        if (
            ...
            raise SystemExit(f"stale A7a figure: {path.relative_to(ROOT)}")
    print("OK: A7a figure hashes are current")
